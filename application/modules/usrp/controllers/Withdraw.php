...
class Withdraw extends Usrp{
    ...
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($list as $dt) {
          ...
          if ($dt->status=="process" OR $dt->status=="cancel") {
            $row[] = '
                        <a href="'.site_url("usrp/withdraw/delete/".enc_url($dt->id_withdraw)).'" class="btn btn-danger btn-sm" title="delete" id="hapus"><i class="fa fa-trash"></i></a>
                     ';
          }else {
            $row[] = "";
          }
  ...
  function add_action()
  {
    if ($this->input->is_ajax_request()) {
      ...
      if ($this->form_validation->run()) {
        ...
        $this->db->insert("withdraw_usrp",$data);
  ...
  function _code()
  {
    $q = $this->db->query("SELECT MAX(RIGHT(code,7)) AS kd_trans FROM withdraw_usrp ORDER BY id_withdraw DESC LIMIT 1");
          $kd = "";
          if($q->num_rows()>0){
  ...
  {
    if ($this->input->is_ajax_request()) {
      $this->db->delete("withdraw_usrp",["id_withdraw" => dec_url($id), "id_penerima_dana"=>sess("id_user")]);
