...
class Withdraw_usrp extends Backend{

  public function __construct()
  {
    parent::__construct();
    $this->load->model("Withdraw_usrp_model","model");
  }

  function index()
  {
    $this->template->set_title("Daftar Withdraw Penerima Dana");
    $this->template->view("content/withdraw_usrp/index");
  }
  ...
  function json()
  {
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($list as $dt) {
          ...
          $row[] = '
                      <a href="'.site_url("backend/withdraw_usrp/detail/".enc_url($dt->id_withdraw)).'" class="btn btn-info btn-sm" title="detail"><i class="fa fa-file"></i></a>
                   ';
  ...
  function detail($id="")
  {
    if ($row = $this->model->get_detail_model(dec_url($id))) {
      $this->template->set_title("Detail Withdraw Penerima Dana #$row->code");
      $data['dt'] = $row;
      $this->template->view("content/withdraw_usrp/detail",$data);
    }
  }
  ...
  if ($this->input->is_ajax_request()) {
        ...
        if ($this->form_validation->run()) {
          ...
          if ($row = $this->model->get_detail_model($id_act)) {
            ...
            $this->model->get_update("withdraw_usrp",$update,["id_withdraw"=>$id_act]);
