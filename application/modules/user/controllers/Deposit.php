...
class Deposit extends User{
  ...
  function json()
  {
    if ($this->input->is_ajax_request()) {
      ...
      foreach ($list as $dt) {
          ...
          if ($dt->status=="process") {
            $row[] = '
                        <a href="'.site_url("user/deposit/detail/".enc_url($dt->id_deposito)).'" class="btn btn-info btn-sm" title="detail"><i class="fa fa-file"></i></a>
                        <a id="hapus" href="'.site_url("user/deposit/delete/".enc_url($dt->id_deposito)).'" class="btn btn-danger btn-sm" title="detail"><i class="fa fa-trash"></i></a>
                     ';
          }else {
            $row[] = '
                        <a href="'.site_url("user/deposit/detail/".enc_url($dt->id_deposito)).'" class="btn btn-info btn-sm" title="detail"><i class="fa fa-file"></i></a>
  ...
  function detail($id)
  {
    if ($row =  $this->model->get_where("deposito",['id_deposito'=>dec_url($id)])) {
      $dt['dt'] = $row;
  ...
  function _code()
  {
    $q = $this->db->query("SELECT MAX(RIGHT(code,7)) AS kd_trans FROM deposito ORDER BY id_deposito DESC LIMIT 1");
          $kd = "";
          if($q->num_rows()>0){
  ...
  function _kode_unik()
  {
    $q = $this->db->query("SELECT kode_unik AS kd_trans FROM deposito ORDER BY id_deposito DESC LIMIT 1");
          $kd = "";
          if($q->num_rows()>0){
              foreach($q->result() as $k){
                $tmp = ((int)$k->kd_trans)+1;
  ...
  {
    if ($this->input->is_ajax_request()) {
      $this->db->delete("deposito",["id_deposito" => dec_url($id), "id_pendana"=>sess("id_user")]);
